...
def SpatialCNN(
    ...
    for layers in range(2, 20):
        with tf.variable_scope('block%d' % layers, reuse=reuse):
            ...
            output = tf.nn.relu(
                tf.layers.batch_normalization(output, training=is_training),
            )
...
class denoiser(object):
    ...
    def evaluate(
        ...
        for idx in range(len(eval_data)):
            ...
            outputimage = \
                np.clip(255 * output_clean_image, 0, 255).astype('uint8')
...
# PSNR function for 0-255 values
...
# PSNR function for 0-1 values
def psnr_scaled(im1, im2):
    mse = ((im1 - im2) ** 2).mean()
    mse = mse * (255 ** 2)
    psnr = 10 * np.log10(255 ** 2 / mse)
